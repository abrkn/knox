...
Client.prototype.request = function(method, filename, headers){
  ...
  headers.Authorization = auth.authorization({
    ...
    , verb: method
    , date: date
    , resource: auth.canonicalizeResource(url.resolve('/', this.bucket + path))
...
Client.prototype.putStream = function (stream, filename, options, fn) {
  var self = this;

    var req = self.put(filename, options);
    req.on('response', function(res){
      fn(null, res);
    });
    stream
...
Client.prototype.putFileStream = function(stream, filename, headers, fn){
  var self = this;
  if ('function' == typeof headers) {
    fn = headers;
    headers = {};
...
Client.prototype.https = function(filename){
  return url.resolve('https://' + this.endpoint, this.bucket + filename);
};
...
Client.prototype.signedUrl = function(filename, expiration){
  ...
  var signature = auth.signQuery(query);
